...
multiple_anova_test<-function(in_data,in_numeric_variable, in_categorical_variable){
    correlation_test<-apply(in_data[,unlist(lapply(in_data, is.numeric))],2,
                                    function(x)anova(x,in_data[,in_categorical_variable]))
    temp<-data.frame(matrix(nrow=length(correlation_test),ncol=6))
        for (i in 1:length(correlation_test)){
            temp[i,]<-c(names(correlation_test)[i], 
                        correlation_test[[i]]$estimate,
...
```{r}
getNumericSummaryTable(in_data=all_data,group_variable='outcome',summary_variable='age')
```
...
The glaring difference of age is also shown below. As you can see, the people with a negative status `r (ad_age_table[1,2] - ad_age_table[2,2])%>%round(3)` younger than those with a positive one.
...
a1<-all_data%>%
    ...
    labs(title="Histogra, Age Distribution Grouped by Disease Status")   
a2<-all_data%>%
    dplyr::select(outcome,age)%>%
