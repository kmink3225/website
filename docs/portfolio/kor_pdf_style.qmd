...
* 기술적 문제점
  * 알고리즘 문서 부재, 코드 주석 부재 및 가독성이 매우 낮은 변수명의 Matlab 코드 포팅 어려움
  ...
  * 복잡한 조건문에서 생성된 근거가 불명확한 50+개의 경험적 파라미터 존재
* 운영적 도전과제
  * 과적합 위험: 특정 장비나 데이터셋에 맞춰진 구체적 조건문들의 새로운 환경에서의 실패 가능성
  * 테스트 어려움: 모든 조건 조합 테스트의 사실상 불가능성과 특정 조건에서만 발생하는 버그 발견의 어려움
  * 예측 불가능성: 유사한 입력 데이터가 미세한 차이로 완전히 다른 보정 경로를 따르는 결과 일관성 부족
  * 동료 data scientist와의 소통 어려움: 통계 비전공자 (물리학자, 컴퓨터 과학자)에게 결정론적 규칙의 한계, 편향누적, 민감도 분석, 결합 추정 등 통계 개념 이해 어려움
  * BT 실무자들의 조직적 저항: 레거시 알고리즘으로 시약의 성능 조절로 시약 개발을 하여 알고리즘 개선 방법론에 대한 수용 거부
* 제약 조건
  * 확장성 문제: 새로운 노이즈 패턴 발견 시마다 조건문 추가로 인한 복잡도의 기하급수적 증가
  * 지식 전달 장벽: 알고리즘 개발자 외 전체 로직 이해 어려움과 기술 이전의 제한성
