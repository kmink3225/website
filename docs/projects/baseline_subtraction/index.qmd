...
* Log Phase (로그 단계): 증폭된 검체의 신호가 일정 농도 이상일 때, PCR 장비에서는 신호의 변화가 나타나며, 포화 상태에 이를 때까지 신호가 2의 지수승으로 증가한다.
* Plateau Phase (플라토 단계): 효소의 활성도 감소 및 반응물의 고갈 등으로 인해 포화 상태에 도달하면, 신호는 더 이상 증가하지 않고 일정하게 유지된다.
...
noise를 보정하는 대표적인 전처리 방식 중 하나인 baseline 차감 알고리즘을 최적화한다. Baseline의 차감은 baseline fitting을 수행하여 fitted value를 raw data에서 차감한다. 

$$
\text{RFU (or PCR Signal)} =  \text{Signal} + \text{Noise} + \text{Error}
$$

* Signal: 시약에 의한 순수 증폭 및 미증폭 신호
...
회사에 처음 입사했을 때, 수많은 if문과 설정값, 그리고 해석하기 어려운 코드에 압도되었다. rule-based 알고리즘이 data-driven 알고리즘에 비해 성능이 떨어진다고는 할 수 없지만, 변화무쌍한 PCR 신호를 조건문으로 처리하기에는 한계가 있다고 판단했다. 따라서, 점진적으로 진단 알고리즘을 data-driven 방식으로 개선할 필요가 있다고 생각했다.  
...
* 기존 및 새로운 baseline fitting 알고리즘들의 성능을 평가 및 비교한다.
    * 알고리즘 1 (rule-based): 자사의 기존 legacy baseline fitting 알고리즘 named 'Legacy'
    * 알고리즘 2 (rule-based): 자사의 biologists들이 실험 설정값을 산출해내기 위해 사용되는 baseline fitting 알고리즘 named 'Bio'
    * 알고리즘 3 (black box): 업계 1위 타사의 black box인 baseline fitting 알고리즘 named 'Black Box'
    * 알고리즘 4 (rule-based): 자사의 신제품에 특이적인 증폭신호에 최적화된 baseline fitting 알고리즘 named '신제품'
    * 알고리즘 5 (rule-based): 알고리즘4 를 개선한 새로운 조건문이 추가된 rule-based 알고리즘 named '신제품 +'
    * 알고리즘 6 (data-driven): machine learning을 이용한 baseline fitting 알고리즘 named 'ML'
