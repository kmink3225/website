...
raw_X = negative_data['preproc_rfu'].iloc[0] # (N,D) = (45,1)
raw_X_min, raw_X_max = normalize(raw_X)[1], normalize(raw_X)[2]

X = normalize(X)[0]

#
#phi1 = lambda x: x**0 # 0제곱 x[0] 대괄호를 씌운 이유는 X가 2차원 array이기 때문에  스칼라를 뽑아내기 위함임
#phi2 = lambda x: x**1 # 1제곱
#phi3 = lambda x: np.e**x
#phi4 = lambda x: np.e**(-x)
#phi5 = lambda x: np.e**(-0.4*x)-np.e**(0.2*x)
#phi6 = lambda x: np.e**(0.4*x)-np.e**(-0.2*x)
#phi7 = lambda x: x*np.e**(-0.4*x)
#
#PHI = np.array([[phi1(x), phi2(x), phi3(x), phi4(x), phi5(x), phi6(x), phi7(x)] for x in X])
#
phi1 = lambda x: x**0 # 0제곱 x[0] 대괄호를 씌운 이유는 X가 2차원 array이기 때문에  스칼라를 뽑아내기 위함임
phi2 = lambda x: x**1 # 1제곱
phi3 = lambda x: x**2
phi4 = lambda x: x**4
PHI = np.array([[phi1(x), phi2(x), phi3(x), phi4(x)] for x in X])
...
y= np.dot(w.T,PHI.T)
denorm_y = denormalize(y_pred[0],raw_X_max,raw_X_min)
raw_X
...
# 첫 번째 데이터 라인 플롯
plt.plot(raw_X, label='raw_X', marker='o')

# 두 번째 데이터 라인 플롯
plt.plot(denorm_y, label='denorm_y', marker='s')
...
def CGM(f, df, x, args=(), eps=1.0e-7, max_iter=2500, verbose=False, callback=None):
    """
    f       : 최적화 할 함수
    df      : 최적화할 함수의 도함수
    x       : 초기값
    ...
    for k in range(max_iter):
        ...
        if k == 0 :
            ...
            # Conjugate gradient Polak and Ribiere
            # beta = (np.dot(c, c-c_old)) / np.linalg.norm(c_old)**2
            d = -c + beta*d

        alpha = gss(f, df, x, d, args=args)[0]
...
negative_data = merged_data.query("`final_ct` < 0")
raw_X = negative_data['preproc_rfu'].iloc[0]  # (N,D) = (45,1)
X, raw_X_min, raw_X_max = normalize(raw_X)

# 기저 함수 정의
phi1 = lambda x: x**0  # 0제곱
phi2 = lambda x: x**1  # 1제곱
phi3 = lambda x: x**2  # 2제곱
phi4 = lambda x: x**3 # 3제곱
phi5 = lambda x: x**4 # 3제곱
phi6 = lambda x: x*np.e**(-x) # 3제곱

# 각 기저 함수 적용
PHI = np.array([[phi1(x), phi2(x), phi3(x), phi4(x), phi5(x), phi6(x)] for x in X])

# 가중치 생성 및 예측값 계산
w = np.random.rand(6).reshape(-1, 1)
y = np.dot(w.T, PHI.T)[0]

# 예측값 복원
denorm_y = denormalize(y, raw_X_min, raw_X_max)
...
def J(w, P, x, y):
    ...
    y_pred = np.dot(w.T, PHI.T) #(N,): w가 1개일 경우 or (K,N): w가 k개일 경우
    # w=(2,K) -> w.T=(K,2), PHI-(N,2) -> PHI.T=(2,N) 
    # np.dot(w.T, PHI.T)=(K,N)
    # 데이터 N개 에 대해서 다 더한다.[+]
    return 0.5*(((y-y_pred)**2).sum(axis=-1)) # 마지막 존재하는 차원 방향으로 summation
...
def grad(w, P, x, y):
    ...
    # PI: (N,M)
    PHI = np.array([ x**i for i in range(P+1) ]).T
    # (K,M)(M,N)(N,M) - (1,N)(N,M) = (K,M)
    g = np.dot(w.T, np.dot(PHI.T, PHI) ) - np.dot(y.T, PHI) #broadcasting
...
W.append(
    CGM(J, grad, w, args=(6, X, y_train), verbose=True)
)
...
# CGM 함수는 이전에 정의된 대로 사용

# 초기 가중치 설정
w = np.random.uniform(-1, 1, 6)

# CGM을 사용하여 가중치 최적화
optimized_w = CGM(J, grad, w, args=(5, X, y), verbose=True)

# 최적화된 가중치를 사용하여 예측
y_pred = np.dot(optimized_w.T, PHI.T)

# 예측값 복원
denorm_y_pred = denormalize(y_pred, raw_X_min, raw_X_max)

plt.figure(figsize=(12, 6))
plt.plot(raw_X, label='Original Data')
plt.plot(denorm_y_pred, label='Predicted Data')
plt.legend()
...
# 오차 계산
mse = np.mean((raw_X - denorm_y_pred)**2)
