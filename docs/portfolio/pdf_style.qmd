...
# [Real-Time PCR 진단 시스템을 위한 지능형 신호 처리 프레임워크]
...
# [Alzheimer's Disease Biomarker 규명을 위한 대사체 분석 연구]

## 개요

* 2차 세계 대전 이후 Baby Boomer Generation의 대규모 치매 발병에 대비한 치매 인자 규명
* 모집단: 장수마을에 거주하는 백인 참여자 (LLFS - Long Life Family Study)
* 기간: 2018.12 - 2020.04 (17개월)
* 역할: Research Statistician
* 참여 인원: Epidemiologist 3명, 생물통계학자 3명, 유전통계학자 1명, 신경외과 의사 4명, 생화학자 2명, 임상연구 코디네이터 8명, 데이터 엔지니어 2명

## 주요 문제점 및 도전과제

* 의학적 문제점
  * 알츠하이머병은 증상 발현까지 20년간 진행되어 전임상 단계 생리학 이해 필요
  * 유전적 요인이 알츠하이머병에 50% 기여하나 구체적 메커니즘 불명
  * APOE 유전자 보유자의 대사체 변화 패턴 미규명
* 데이터 분석적 도전과제
...
* 통계분석: Linear regression, Logistic regression, Cox Proportional Hazards regression, Linear Mixed Effect Model, GWAS
...
* 통계적 검증: Permuted p-values를 통한 multiple correction
...
* 연구 성과
  * 약 3,000개 변수 중 60개 내외의 대사물질이 알츠하이머병과 5% 유의수준에서 유의한 관련성 확인
  * Partial Least Squares가 최적 성능의 분류기로 선정
  * EDA 및 Data Mining을 통한 미지의 강력한 교란자 규명
  * 주 3회 분석결과 공유 Sprint
  * Metabolic Pathway Map 제시

![EDA & Data Mining 결과](./data/LLFS_datamining.PNG){width=40%}

![일부 GWAS 결과](./data/LLFS_gwas.PNG){width=30%}

* 학술적 성취
  * Columbia University Mailman School of Public Health 연례 연구 발표회 포스터 발표
  * 연례 연구 경진대회에서 약 100명의 대학원생 중 상위 3명 선정, 상금 $1,000 수상
  * 학과장상 수상
  * Columbia University Irving Medical Center 신경외과 정규직 Job Offer 획득
* 방법론적 기여
  * 고차원 대사체 데이터에 대한 체계적 분석 파이프라인 구축
  * 다중 센터 데이터의 품질 관리 프로토콜 확립
  * 알츠하이머병 바이오마커 발굴을 위한 통합 분석 방법론 개발

## 기대효과

* 연구 방법론적 영향
  * 고차원 생체분자 데이터 분석을 위한 통합 파이프라인 방법론 확립
  * 다중 센터 연구에서의 데이터 품질 관리 표준 제시
  * 다학제 협업 연구 모델 및 분석 방법론 제안
* 알츠하이머병 치료제 개발을 위한 타겟 발굴 기여
* 정밀의학 기반 치매 예방 및 관리 체계 구축 지원
* 노인성 질환 연구 분야의 데이터 사이언스 방법론 발전 기여
