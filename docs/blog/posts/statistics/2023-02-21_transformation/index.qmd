...
이번 블로그에서는 주어진 확률 변수 $X$ 에 대해서 $X$ 의 pmf 또는 pdf $f_x(x)$ 를 알고있을 때 확률 변수에 $X$ 에 적절한 함수의 변환을 적용해 확률 변수 $Y$ 를 $Y=u(X)$ 라는 관계식이 정의 가능할 때 $Y$ 의 pdf를 구하는 방법에 집중한다. 후에 MGF (Momment Generating Function)에 응용될 수 있는 개념으로 잘 정리할 필요가 있다. 
...
동전을 독립적으로 2번 던질 때, 확률 변수 $X$ 는 앞면이 나오는 수라고 했을 때, 확률 분포는 다음과 같다. 이 때 $Y=2X+1$ 라는 관계가 성립할 때 $Y$ 의 분포는 다음과 같다.
...
동전을 독립적으로 2번 던질 때, 확률 변수 $X$ 는 앞면이 나오는 수가 짝수일 때, 확률 분포는 다음과 같다. 이 때 $Y=mod(X,2)$ 라는 관계가 성립할 때 $Y$ 의 분포는 다음과 같다.
...
* When it is not a One-to-One relation (another example)
...
what is $f_Y(y)$ when $f_X(x)=\lambda e^{-\lambda x}$?
