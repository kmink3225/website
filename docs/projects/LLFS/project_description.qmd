...
The objective of the Long Life Family Study (LLFS) project was to build an analysis pipeline of identifying significant biomarkers for AD using statistics and machine learning at the multi-stages from the genomic to the metabolomic stage through the transcriptomic and proteomic stage. The project described the process I actually carried out the project, and the self project described the methodology in a roughly simplified form.
...
Data analysis was performed in three stages: Exploratory Data Analysis (EDA), Statistical Analysis, and Machine Learning. 
An analysis was performed to ensure that the results from each stage were consistent at each stage.
