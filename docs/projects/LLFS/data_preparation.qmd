...
MVN에 의해 만들어진 Data와 미리 만들어 놓은 가중치 Matrix의 곱을 통해 Score Matrix를 만들고 Logit Link를 이용하여 각 Sample의 확률값을 만들었다. 각 Sample의 확률값을 기반으로 $X ~\text{Bernoulli}(p)$, (여기서 $p$는 각 sample이 갖는 확률값 뜻한다), 을 통해 disease status의 정보를 담은 binary outcome variable를 만들었다. 

**Sex Variable Setting**

Sex 변수는 $X ~\text{Bernoulli}(0.5)$ 을 통해 data를 생성했다.
