...
Let $f: X \to \mathbb{R}$ be a real-valued function defined on a domain $X \subseteq \mathbb{R}$, and let $x^* \in X$ be a point in $X$. We say that $x^*$ is a local minimizer of $f(x)$ if there exists a radius $\delta>0$ of a neighborhood $N(x^*)$ of $x^*$ or $|x-x^*|<\delta$ such that $f(x^*) < f(x)$ for all $x \in N(x^*) \cap X$.

:::

:::{#def-strict_local_maximizer}

Let $f: X \to \mathbb{R}$ be a real-valued function defined on a domain $X \subseteq \mathbb{R}$, and let $x^* \in X$ be a point in $X$. We say that $x^*$ is a local maximizer of $f(x)$ if there exists a radius $\delta>0$ of a neighborhood $N(x^*)$ of $x^*$ or $|x-x^*|<\delta$ such that $f(x^*) > f(x)$ for all $x \in N(x^*) \cap X$.
