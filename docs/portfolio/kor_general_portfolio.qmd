  ...
  pdf:
    ...
    include-in-header:
      - text: |
          ...
          % Make the table of contents single-column (surrounded by page breaks)
          \pretocmd{\tableofcontents}{\clearpage\onecolumn}{}{}
          \apptocmd{\tableofcontents}{\clearpage\twocolumn}{}{}
---

# 프로필 {.unnumbered}

6년+ 경력의 데이터 사이언티스트로서 머신러닝, 자연어 처리, GenAI 응용하여 다수의 비즈니스 임팩트 창출 실적을 보유하고 있습니다. 데이터 사이언스, 생명공학, 엔지니어링 등 다학제 분야에 걸친 20명 규모 팀을 리드하여 엔터프라이즈급 RAG 프로토타입과 데이터 거버넌스 플랫폼을 구축하고 있습니다. 실험설계, 인과추론, 통계적 검증 전문가이며 9건의 특허(제1발명가 5건) 보유. 규제 환경(FDA 컴플라이언스)에서 규칙 기반 알고리즘을 데이터 기반 알고리즘으로 전환 경험이 있습니다.


## 핵심 전문분야 {.unnumbered}

데이터 분석, 실험 설계, RAG/LLM 시스템, 통계 모델링, 크로스펑셔널 또는 다학제 리더십, 데이터 거버넌스, 임상 데이터 분석
...
Senior Data Scientist (프로젝트 리드 경험)

### 핵심 역량 {.unnumbered}

데이터 거버넌스(NLP), 검증 자동화(FDA), 신호처리 ML/DL, RAG, 분석 파이프라인/문서 자동화, 시스템 아키텍처 설계(NLP·RAG 표준화/거버넌스·검증 파이프라인)

### 핵심 프로젝트 {.unnumbered}

- 자연어처리(NLP) 기반 데이터 거버넌스 시스템 구축 (2024.10 ~ 2025.08)
- RT PCR 진단 시스템을 위한 지능형 신호 처리 알고리즘 개발 (2024.01 ~ 2024.09)
- 진단 알고리즘 안전성 검증 시스템 구축 (2023.05 ~ 2023.12)
- 레거시 Rule Based 알고리즘을 Data Driven 알고리즘으로 전환 (2021.10 ~ 2023.04)
- 진단 장비 QC 자동화 시스템 구축 및 알고리즘 고도화 (2020.12 ~ 2021.09)
- 치매 Biomarker 규명 및 머신러닝 방법론 비교 연구 (2018.12 ~ 2020.05)
...
![Report Table](./data/fda_table.PNG){width=25%}

![Overview Plot](../../images/projects/fda_validation/result_plot.PNG){height=28%}

![Detailed Plot](./data/fda_plot.PNG){height=28%}
...
* 메카니스틱 모델의 투명성
  ...
  * 로지스틱 시그모이드 일반형의 파라미터들($L_{\min}, L_{\max}, k, x_0$)이 각각 baseline 신호 하한, 포화 신호 상한, 증폭률(기울기), 변곡점 등 물리적 의미를 가짐
  * 합성함수의 각 구성요소($g_1, g_2, g_3$​)가 신호 전처리 단계와 일대일 대응되어 "왜 이런 결과가 나왔는가"를 생물학적 메커니즘으로 설명 가능
...
![개선된 QC 프로세스](./data/qc_as_is.png){height=30%}
...
![LSTM ROC Curve](./data/qc_roc_curve.png){width=40%}
...
![EDA: Correlation Plots between Metabolites and Phenotype](./data/LLFS_datamining.PNG){width=45%}

![일부 GWAS 결과](./data/LLFS_gwas.PNG){width=30%}

![ML Classifier Performance](./data/LLFS_classfier_performance.PNG){width=45%}
