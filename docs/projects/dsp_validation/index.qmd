...
* COVID19 특수 시기 해제 후 Global market으로 진입 및 유지를 위해 제품의 안정성 검증이 각 국의 정부로 부터 요구된다. 
* EU(European Union)의 경우 IVDR (In Vitro Diagnostics Regulation) 을 요구하고 그 중 가장 엄격한 기준을 요구하는 미국의 FDA와 캐나다의 Health Canada의 surveilance 기준으로 진단 알고리즘의 안정성 검증 문서를 기획하고 작성한다.
...
* 따라서, Diagnostic Algorithm의 안정성 검증은 Software Engineering Testing과 Advanced Testing을 포함한다. 여기서 Advanced Testing은 Statiscal Analysis에 기반한 Statistical Testing을 의미하고 안정적인 Software Engineering System 구축이 그 전제가 된다.
...
* BT (Biotechnology) 부문과 IT (Information technology) 부문의 협력이 전제가 되어야하며 BT 부서의 experiment design 및 limitation factors at a experimental level을 고려한 engineering design와 statistical design을 확립한다.
...
* BT부서와의 소통으로 업무 문서화 진행하여 실험 결과의 기대 정답 기준 확립, 독립 변수 및 종속 변수 확립
...
* Seegene 고유의 software testing model 기획 및 확립 후 statistical analysis 기획 및 수행
...
* In order to enter and maintain the global market after the COVID19 special period is lifted, product stability verification and validation is required from the government of each country.
* In the case of the EU (European Union), IVDR (In Vitro Diagnostics Regulation) is required, and the safety verification document of the diagnostic algorithm is planned and prepared according to the surveilance standards of the US FDA and Canada's Health Canada, which require the most stringent standards among them.
...
* Includes both Structural Testing and Advanced Testing. 
* Establish a definition and logic for algorithm safety.
* Establish metrics or indicators for algorithm safety.
* Statistical Testing, which is Advanced Testing, is a task that requires the creativity of a data scientist, and statistical analysis design is performed by planning a testing model.
...
* Plan and perform statistical analysis after planning and establishing Seegene's own algorithm testing model
