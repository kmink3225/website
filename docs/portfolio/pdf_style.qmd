...
* 기술적 문제점
  * 신호 노이즈 복잡성: 미시적 화학/광학/기계적 반응으로 인한 예측 불가능한 노이즈 패턴
  * 알고리즘 분산화: 사내 여러 baseline fitting 알고리즘 병존으로 인한 비표준화 및 소통 장애
...
* 제약 조건
  ...
  * 적은 데이터 포인트: 제한적인 baseline 데이터에서도 동작하는 robust 알고리즘 필요
...
* 시스템 개선
  ...
  * 시각화 도구: 비전문가도 이해 가능한 직관적 성능 비교 대시보드
* 프로세스 혁신
