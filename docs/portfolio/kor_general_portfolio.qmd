---
title: "Data Scientist / Applied ML Scientist Portfolio"
  ...
  pdf:
    ...
    include-in-header:
      - text: |
          ...
          \usepackage{cuted}
          \renewcommand{\contentsname}{프로젝트 목차}
          \newif\iffirstsection
          \firstsectiontrue
          % Clear page from second H1 onward, and keep header with at least 5 lines
          \pretocmd{\section}{\iffirstsection\global\firstsectionfalse\else\clearpage\fi\Needspace{5\baselineskip}}{}{}
          % (Removed single-column TOC page breaks to allow in-column TOC)
---
...
# [NLP를 활용한 Data Governance 시스템 단계적 구축]{#proj-governance}

**So-What: 전사 데이터 표준화 체계를 확립해 데이터 품질과 의사결정 속도를 동시에 향상.**
...
![표준화 프레임워크 아키텍처(단계적 확장·자동화 기반)](./data/governance_nlp.PNG){height=45%}
...
* 자연어 처리: PyTorch (Hugging Face Transformers, KoBERT, KoSRoBERTa)
...
* 시스템 구축 성과
  ...
  * NLP(KoSRoBERTa)를 통한 유사 용어 Clustering:  금칙어 관리 및 표준안 관리
...
# [Real-Time PCR 진단 시스템을 위한 지능형 신호 처리]{#proj-pcr-signal}

**So-What: 불확실한 신호에도 견고한 baseline 처리로 위음성률을 산업 최고 수준으로 개선.**
...
![엔드투엔드 신호 파이프라인(검증·시각화 자동화)](./data/baseline_subtraction_pipeline.PNG){width=45%}

2. 알고리즘 비교 분석

* 1st Panel [After BPN]: normalized **Raw Data**를 보여준다. 
...
![복수 신호에서 ML이 White noise 근접(최우수) 성능 입증](./data/multiple_signal_evaluation.PNG){width=45%}

![특이 신호에서 Gray zone 판독 안정성 개선 입증](./data/single_signal_evaluation.PNG){width=45%}
...
# [북미 진출을 위한 진단 알고리즘 안전성 검증 자동화]{#proj-fda-validation}
...
![검증 자동화 파이프라인(레포트 6개월→3주)](../../images/projects/fda_validation/data_pipeline.PNG){width=45%}
...
![FDA용 통계표 자동 생성(검증 재현성 강화)](./data/fda_table.PNG){width=25%}

![전반 성능 지표 시각화(이해관계자 커뮤니케이션 향상)](../../images/projects/fda_validation/result_plot.PNG){height=28%}

![모듈별 성능 비교(리스크 포커스 분석)](./data/fda_plot.PNG){height=28%}
...
# [레거시 Rule-Based 알고리즘의 Data-Driven 전환]{#proj-legacy-to-ml}

**So-What: 설명가능·검증가능한 하이브리드 모델로 레거시 한계의 구조적 해결 방향 제시.**
...
$$
f(x; \phi_1, \phi_2, \phi_3, \beta) = g_3(g_2(g_1(x, \phi_1), \phi_2), \phi_3) + \text{sigmoid}_{g}(x; L_{\min}, L_{\max}, k, x_0)
$$

일반형 로지스틱 시그모이드 정의(예시):

$$
\text{sigmoid}_{g}(x; L_{\min}, L_{\max}, k, x_0) = L_{\min} + \frac{L_{\max} - L_{\min}}{1 + e^{-k(x - x_0)}}
$$
...
### 규제 환경에서의 설명력(Explainability) 근거 {.unnumbered}
...
# [진단 장비 QC 프로세스 자동화 및 알고리즘 고도화]{#proj-device-qc}
...
![기존 수동 QC 프로세스(병목·왜곡 발생)](./data/qc_as_was.png)
...
![개선된 자동화 QC 프로세스(QC시간 14배 단축)](./data/qc_as_is.png){height=30%}
...
![장비 성능 클러스터링(등급화·공급 최적화 근거)](./data/clustering_result.PNG){width=30%}

* LSTM을 활용한 Step 1 데이터를 통한 Step 2 결과 예측 모델 개방
  * 합격/불합격 분류 정확도: 94.5%
  * 장비 성능 등급 분류 정확도: 82.7%

![LSTM 혼동행렬(합불 94.5% 정확도)](./data/qc_confusion_matrix.png){width=48%}

![LSTM ROC(등급 분류 82.7% 정확도)](./data/qc_roc_curve.png){width=40%}
...
# [치매 Biomarker 규명: 대사체 통계 분석 및 머신러닝 방법론 비교 연구]{#proj-llfs}

**So-What: 고차원·소표본 환경에서 임상 타당성 높은 후보를 통계·ML 융합으로 도출.**
...
![다층 오믹스 데이터 파이프라인(품질관리·전처리 체계)](./data/LLFS_overview.PNG){height=30%}
...
* Cox PH Model 선택: 알츠하이머병의 본질적 특성(시간 의존, right censoring, semi-parametric) 반영
...
![질환-대사체 상관 구조 탐색(강력 교란자 규명)](./data/LLFS_datamining.PNG){width=45%}

![GWAS로 유의 유전자 규명(생물학적 타당성 강화)](./data/LLFS_gwas.PNG){width=30%}

![ML 분류기 성능 비교(최적 sPLS, 84% 정확도)](./data/LLFS_classfier_performance.PNG){width=45%}

![ML Classifier ROC(분류 성능 시각화)](./data/LLFS_ROC.PNG){width=40%}
