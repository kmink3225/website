...
```{r}
raw_data=read_csv("data/excel.practice.csv")
names(raw_data)
raw_data=raw_data%>%rename(id=1)
raw_data[,-c(1:15)]%>%ncol

mean_sd_data<-rbind("mean"=sapply(raw_data[,-c(1:15)],function(x)mean(x)),
      "sd"=sapply(raw_data[,-c(1:15)],function(x)sd(x)))

mean_sd_data%>%as.data.frame()

sample_size <- 500
variable_size <- 10000 # the number of predictors
significant_variables <- variable_size*sample((50:200)/1000,1) # the number of predictors associated with a response variable

metabolites <- matrix(rnorm(sample_size*variable_size), 
            nrow=sample_size, ncol=variable_size)%>%as.data.frame()
