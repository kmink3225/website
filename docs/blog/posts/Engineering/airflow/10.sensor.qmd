...
* [airflow.sensors.filesystem](https://airflow.apache.org/docs/apache-airflow/stable/_api/airflow/sensors/filesystem/index.html#airflow.sensors.filesystem.FileSensor)
  ...
  * Parameters
    * fs_conn_id – reference to the File (path) connection id (connection을 airflow web에 미리 등록해놔야함)
  ...
  * source: 모든 sensor는 poke만 잘 이해하면 거의 이해함
  ```markdown
    ...
    [docs]    def poke(self, context: Context):
            hook = FSHook(self.fs_conn_id) # file sensor를 쓰기전에 file system과 관련된 connection을 사전에 설정해놔야함
  ...
  * `FSHook()` : [명세서](https://airflow.apache.org/docs/apache-airflow/stable/_api/airflow/hooks/filesystem/index.html)
    ```markdown
