...
* 역할: Data Standardization & Technical Lead
...
* 표준화 프레임워크 구축: 표준화 원칙 및 표준 사전 구축
  * 문제: 표준화 원칙 및 표준 사전 구축
  * 해결방안: 표준화 현황 분석 및 표준화 프레임워크 확립
* 복잡한 명명 규칙의 프로그래밍 구현
  * 문제: 16단계 영문 약어 생성 규칙 구현
  * 해결방안: 계층적 Rule Engine 설계
* 대규모 데이터의 품질 평가 자동화
  * 문제: 수천 개 테이블/컬럼에 대한 KPI (품질 평가 지표) 산출
  * 해결방안: 원칙 기반 평가 지표 개발
* BT & IT 용어 표준화
